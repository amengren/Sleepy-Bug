...
public:
    ...
    void moveToIndexPath(SNSIndexPath* indexPath, bool _animation=false);
    
    //dhd
protected:
    //是否默认就实例化所有数据
    CC_SYNTHESIZE(bool, m_isAllMaxInstance, AllMaxInstance);
    //设置移动动画时间
    CC_SYNTHESIZE(float, m_moveAnimationDuration, MoveAnimationDuration);
};
